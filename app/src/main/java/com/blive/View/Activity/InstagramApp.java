...
public class InstagramApp {
	...
	public String user,name,id,domain,username;
	...
	public InstagramApp(Context context, String clientId, String clientSecret, String callbackUrl) {
		...
		mAuthUrl = AUTH_URL + "?client_id=" + clientId + "&redirect_uri=" + mCallbackUrl+ "&response_type=code&scope=basic";
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					writer.flush();
					String response = com.blive.View.Activity.Utils.streamToString(urlConnection
							.getInputStream());
					...
					Log.i(TAG, "Got access token: " + mAccessToken);
					 ...
					 id = jsonObj.getJSONObject("user").getString("id");
					user = jsonObj.getJSONObject("user").getString("username");
					 name = jsonObj.getJSONObject("user").getString(
							"full_name");
	...
	public void fetchUserName(final Handler handler) {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					 url = new URL(API_URL + "/users/" + mSession.getId() + "/?access_token=" + mAccessToken);
					...
					urlConnection.setDoInput(true);
					urlConnection.connect();
					String response = com.blive.View.Activity.Utils.streamToString(urlConnection.getInputStream());
			 ...
			 username=	userInfo.put(TAG_USERNAME, data_obj.getString(TAG_USERNAME));
